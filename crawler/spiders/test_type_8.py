...
class TestType8(Spider):
    """shitibiao 76：长篇阅读（内容理解·长篇 / reading_long）。

    结构与中篇阅读（type_7）同构：一段较长文章（多个 <p>）+ N 个问句（每问一个 icheck
    组、4 选项）。真实问数动态——从 icheck1 起数非占位组，遇「请勾选」占位（永远在末尾）即止。
    长篇问数常多于中篇（样本见 4 问），动态计数天然支持任意问数。无年份的篇跳过。

    文章含下划线词（问句会引用，如「①何とも複雑な心境になる」）：<u>…</u> 转成统一标记
    ...
    def parse(self, response):
        ...
        # 日期格式形如 2013-12-N1；不能用宽松的 20\d\d，否则会误匹配问句里的「2010年」或
        # 页脚 copyright「2013-2026」。要求紧凑格式「20XX-XX...N级」，从后往前找（日期总在 meta 末尾）。
        date = ""
...
if __name__ == '__main__':
    ...
    spider = TestType8(username=require("SPIDER_USERNAME"), password=require("SPIDER_PASSWORD"))
    spider.run()
