...
"""
from backend.config.categories import category_name, category_order
...
def build_exam(
    ...
    # 整场组卷：还原标准卷面顺序——按题型标准序（文字词汇→语法→阅读），同题型内按原始题号。
    if unlimited:
        ordered_ids = _order_by_exam_layout(cur, ordered_ids)
...
def _order_by_exam_layout(cur, group_ids: list[int]) -> list[int]:
    """把题组按 JLPT 标准卷面顺序排列：题型标准序 → 同题型内原始题号（source_ref 尾部数字）。"""
    placeholders = ", ".join(["%s"] * len(group_ids))
    cur.execute(
        f"SELECT id, category, source_ref FROM question_groups WHERE id IN ({placeholders})",
        group_ids,
    )
    meta = {r["id"]: (r["category"], r["source_ref"]) for r in cur.fetchall()}

    def _orig_no(source_ref: str | None) -> int:
        # source_ref 形如 result_71_validated#12 → 取 # 后数字作原卷内题号
    ...
    def _key(gid):
        cat, ref = meta.get(gid, (None, None))
        return (category_order(cat), _orig_no(ref))

    return sorted(group_ids, key=_key)
