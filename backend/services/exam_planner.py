...
MAX_TOTAL = 50
DEFAULT_TOTAL = 10
# 整场考试组卷的题量上限（一场约 46~54 题组，留足余量）
WHOLE_EXAM_CAP = 300
_DATE_RE = re.compile(r"20\d\d-\d\d")
_DATE_CN_RE = re.compile(r"(20\d\d)\s*[年\-/.]\s*(\d{1,2})")


def _norm_exam_date(raw) -> str | None:
    """把日期归一化成 'YYYY-MM'，兼容 '2010-07' 与 '2010年7月' 两种写法。"""
    if not isinstance(raw, str):
        return None
    m = _DATE_RE.search(raw)
    if m:
        return m.group(0)
    m = _DATE_CN_RE.search(raw)
    if m:
...
def _sanitize(raw: dict, level: str | None, available_codes: set[str]) -> dict:
    ...
    # 整场考试日期：形如 2010-07（兼容 "2010年7月"）；非法则忽略
    exam_date = _norm_exam_date(raw.get("exam_date"))
    whole_exam = bool(raw.get("whole_exam")) and exam_date is not None

    # 整场模式题量上限放宽，其余按 MAX_TOTAL 封顶
    cap = WHOLE_EXAM_CAP if whole_exam else MAX_TOTAL

    # 题型配额：仅保留合法且该级别可用的 code，count 夹在 1..cap
    ...
    # 配额总数封顶
    if clean_quotas:
        while sum(clean_quotas.values()) > cap:
    ...
    try:
        total = max(1, min(int(total), cap))
    except (TypeError, ValueError):
        total = WHOLE_EXAM_CAP if whole_exam else DEFAULT_TOTAL
...
5. 若用户想按"某年某月的考试/某场真题/整套真题"组卷（如"2010年7月的所有题"、"2013年12月的题"），
   请输出 exam_date（格式 "YYYY-MM"，如 "2010-07"）；若是"所有题/整套/全部"则同时 whole_exam=true
   （此时不限 50 题上限，按该场实际题量出）。可与题型叠加（如"2010年7月的语法题"→ exam_date + category_quotas）。
   若用户没提年月，exam_date 留 null、whole_exam 留 false。
6. rationale 用一句中文向用户说明这套卷的组卷思路（会展示给用户）。
